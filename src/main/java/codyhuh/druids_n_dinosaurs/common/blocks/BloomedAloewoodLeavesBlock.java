...
public class BloomedAloewoodLeavesBlock extends LeavesBlock {
    public BloomedAloewoodLeavesBlock(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResult use(BlockState pState, Level pLevel, BlockPos pPos, Player pPlayer, InteractionHand pHand, BlockHitResult pHit) {
        ...
        if (itemstack.canPerformAction(net.minecraftforge.common.ToolActions.SHEARS_CARVE)) {
            if (!pLevel.isClientSide) {
                ...
                pLevel.setBlock(pPos, ModBlocks.ALOEWOOD_LEAVES.get().defaultBlockState(), 11);
                ItemEntity itementity = new ItemEntity(pLevel, (double)pPos.getX() + 0.5D + (double)direction1.getStepX() * 0.65D, (double)pPos.getY() + 0.1D, (double)pPos.getZ() + 0.5D + (double)direction1.getStepZ() * 0.65D, new ItemStack(ModBlocks.BRIGHT_BLOOMS.get(), 4));
