package codyhuh.druids_n_dinosaurs.util.loot;
...
public class AddItemLootModifier extends LootModifier {
    ...
    public static final Supplier<Codec<AddItemLootModifier>> CODEC = Suppliers.memoize(()
            ...
            .fieldOf("item").forGetter(m -> m.items)).apply(inst, AddItemLootModifier::new)));

    private final List<Item> items;

    public AddItemLootModifier(LootItemCondition[] conditionsIn, List<Item> items) {
        super(conditionsIn);
        this.items = items;
    }
